...
import { ThumbsUp, ThumbsDown, MinusCircle, Mail, Share2, FileText, MessageSquare, Check, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { savePosition, getPosition, recordAction, checkAchievements } from "@/lib/gamification";
...
const ACTION_URLS = {
  contact_mep: "https://www.europarl.europa.eu/meps/en/home",
  consultation: "https://ec.europa.eu/info/law/better-regulation/have-your-say_en",
...
const POSITION_CONFIG: Record<Position, { bg: string; bgFull: string; icon: typeof ThumbsUp }> = {
  support: { 
...
export function ActionPanel({ 
  ...
  const [loadingAction, setLoadingAction] = useState<ActionType | "position" | null>(null);
  ...
  const handleShare = async () => {
    ...
    const shareUrl = `${window.location.origin}/procedure/${encodeURIComponent(procedureReference)}`;
    
    try {
      if (navigator.share) {
        await navigator.share({ title: procedureTitle, text: shareText, url: shareUrl });
  ...
  return (
    <div className={containerClasses}>
      {actionFeedback && (
        <div 
          className={`flex items-center gap-2 text-green-600 dark:text-green-400 bg-green-500/10 rounded-md animate-pulse ${isCompact ? "text-xs px-3 py-2" : "text-sm px-4 py-3"}`}
          role="status"
          aria-live="polite"
        >
      ...
      {showPositionForm ? (
        <div className={isCompact ? "space-y-2" : "space-y-3 p-4 bg-muted/50 rounded-lg"}>
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              className={`${isCompact ? "text-xs h-6 px-2" : "text-xs"} text-muted-foreground hover:text-foreground`}
            >
              Cancel
            </Button>
          </div>
          <div className={`flex ${isCompact ? "gap-1.5" : "gap-2"}`}>
            {(["support", "oppose", "neutral"] as Position[]).map((pos) => {
              ...
              const isLoading = loadingAction === "position" && selectedPosition === pos;
              ...
              return (
                <Button
                  ...
                  className={`flex-1 ${focusClasses} ${isCompact ? "py-2 px-2 text-xs" : "py-3 px-3 text-sm"} ${
                    selectedPosition === pos ? config.bg + " border-current" : ""
                  }`}
        ...
        <Button
          ...
          className={`w-full justify-between ${POSITION_CONFIG[existingPosition.position].bg} ${focusClasses} ${isCompact ? "text-xs px-3 py-2" : "text-sm px-4 py-3"}`}
        ...
        >
          <span className="flex items-center gap-2">
            ...
            <span>{isCompact ? "" : "You "}<span className="capitalize">{existingPosition.position}</span>{!isCompact && " this procedure"}</span>
          </span>
        ...
        <Button
          ...
          size="sm"
          onClick={() => setShowPositionForm(true)}
          className={`w-full border-dashed border-primary/30 text-primary hover:bg-primary/5 ${focusClasses} ${isCompact ? "text-xs px-3 py-2" : "text-sm px-4 py-3"}`}
        ...
        >
          ...
          <span className={`${xpTextSize} opacity-70 ml-2`}>+{XP_REWARDS.STATE_POSITION} XP</span>
        </Button>
      )}

      <div className="grid grid-cols-2 gap-2">
        {(["contact_mep", "consultation", "petition", "share"] as ActionType[]).map((actionType) => {
          const config = ACTION_CONFIG[actionType];
          const Icon = config.icon;
          const isLoading = loadingAction === actionType;
          const url = actionType !== "share" ? ACTION_URLS[actionType as keyof typeof ACTION_URLS] : undefined;
          ...
          const colorClasses = {
            ...
            green: "border-green-500/20 bg-green-500/5 hover:bg-green-500/10 text-green-500",
          }[config.color];
          ...
          return (
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => actionType === "share" ? handleShare() : handleAction(actionType, url)}
              disabled={isLoading}
            ...
            >
              ...
              <span className={`${textSize} font-medium text-foreground`}>{config.label}</span>
              <span className={`${xpTextSize} text-muted-foreground group-hover:text-current`}>+{config.xp} XP</span>
            </Button>
          );
        })}
      </div>
