 ...
 *
 * Lives outside the route handler so that `generateMetadata` and the page can
 * reuse it: metadata needs the real title to be useful for search results and
 * link previews.
 */
...
export interface ProcedureDetails {
  ...
  lastActivity?: { date: string; type: string };
  timeline: TimelineEvent[];
  /** True when nothing could be resolved and only the reference is known. */
  isFallback: boolean;
}
...
export async function getProcedureByReference(
  ...
  return {
    ...
    sourceUrl: isFallback ? undefined : getProcedureUrl(reference),
    isFallback,
  };
}
